...
async function transactionsRoutes(app2) {
  ...
  app2.get("/summary", preHandlers, async (request) => {
    const { sessionId } = request.cookies;
    const summary = await knex("transactions").where("session_id", sessionId).sum("amount", { as: "amount" }).first();
    return { summary };
  });
  app2.get("/:id", preHandlers, async (request, response) => {
    const { sessionId } = request.cookies;
  ...
  app2.delete("/:id", preHandlers, async (request, response) => {
    const { sessionId } = request.cookies;
    const getTransactionPramsSchema = import_zod2.z.object({
      id: import_zod2.z.string().uuid()
    });
  ...
  app2.get("/", preHandlers, async (request) => {
    const { sessionId } = request.cookies;
